...
function applyPassengerCapacityLimit() {
  ...
  options.forEach(function (opt) {
    var numeric = parseInt(opt.value, 10); // treats "7+" as 7
    var fits = numeric <= capacity && opt.value.indexOf('+') === -1;
    opt.disabled = !fits;
    if (fits) validValues.push(opt.value);
  });

  var currentNumeric = parseInt(select.value, 10);
  var currentFits = currentNumeric <= capacity && select.value.indexOf('+') === -1;
