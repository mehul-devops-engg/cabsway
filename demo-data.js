...
var CW_DEMO_SEED = {
  ...
  settings: {
    ...
    email: 'rushikeshahire125@cabsway.in',
...
function cwAssertPassengersFitVehicle(db, vehicleModelName, passengers) {
  if (!vehicleModelName || !passengers) return;
  var match = db.vehicles.find(function (v) { return v.model === vehicleModelName; });
  if (!match) return;
...
function cwDemoFallback(action, payload) {
  ...
  switch (action) {
    ...
    case 'createBooking': {
      ...
      cwAssertPassengersFitVehicle(db, payload.vehicleType, payload.passengers);
