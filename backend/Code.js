...
/** Throws if the requested passenger count exceeds the named vehicle model's real capacity. */
function assertPassengersFitVehicle_(vehicleModelName, passengers) {
  if (!vehicleModelName || !passengers) return;
  var vehicles = sheetToObjects(getSheet(SHEET_NAMES.VEHICLES));
  var match = vehicles.filter(function (v) { return v.model === vehicleModelName; })[0];
  if (!match) return; // unknown model name — nothing to validate against
  if (Number(passengers) > Number(match.capacity)) {
    throw new Error('The ' + vehicleModelName + ' seats up to ' + match.capacity + ' — reduce passengers or choose a larger vehicle.');
  }
}
...
function createBooking(payload) {
  ...
  assertPassengersFitVehicle_(payload.vehicleType, payload.passengers);
...
function setupSheets() {
  ...
  createTabIfMissing_(ss, SHEET_NAMES.SETTINGS,
    ...
    [['CabsWay', '+91 91588 18546', 'rushikeshahire125@cabsway.in', 'Nashik, Maharashtra', '', 250, 'images/logo.svg', 'cabsway123']]);
